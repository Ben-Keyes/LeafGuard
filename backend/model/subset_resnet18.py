...
# Using the 10 tomato classes rather than entire dataset
SELECTED_CLASSES = [
...
class FilteredImageFolder(Dataset):
    def __init__(self, root, selected_classes, transform=None):
        base = datasets.ImageFolder(root, transform=None)
        self.transform = transform
        self.loader = base.loader
        self.classes = selected_classes
        self.class_to_idx = {c: i for i, c in enumerate(selected_classes)}
        idx_map = {base.class_to_idx[c]: i for c, i in self.class_to_idx.items()}
        self.samples = [
            (path, idx_map[label])
...
if __name__ == '__main__':
    ...
    patience = 2
    patience_counter = 0
    best_model_path = "leaf_resnet18_best_weighted_subset.pth"
